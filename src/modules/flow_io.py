"""
 flow_io.py
  ...
  file from the Spring Benchmark: https://spring-benchmark.org/
  Modified by Julien Zouein on 18/03/2026.
----------------------------------------------------------------------------
...
def writePngFlow(flow, filename):
    ...
    with open(filename, "wb") as f:
        writer = png.Writer(width=width, height=height, bitdepth=16, greyscale=False)  # type: ignore[reportArgumentType]
        writer.write(f, flow)
...
def readFlo5Flow(filename):
    with h5py.File(filename, "r") as f:
        ...
        return f["flow"][()]  # type: ignore[reportIndexIssue]
...
def writePngDisp(disp, filepath):
    ...
    with open(filepath, "wb") as f:
        writer = png.Writer(width=width, height=height, bitdepth=16, greyscale=True)  # type: ignore[reportArgumentType]
        writer.write(f, disp)
...
def readDsp5Disp(filename):
    with h5py.File(filename, "r") as f:
        ...
        return f["disparity"][()]  # type: ignore[reportIndexIssue]
